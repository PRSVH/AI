...
def every_bird_heuristic(state: State,
    ...
    yellowbirds= list(yellow_birds)
    distancegoal = problem.distance
    while len(yellowbirds)>0:
        dist_dict = dict()
        goal =None
        for bird in yellowbirds:
            dist_dict[bird] = distancegoal[currentbird,bird] #we update this dictionary every time that a bird gets eaten
        minval= min(dist_dict.values())
        for elem in dist_dict:
            if dist_dict[elem]==minval: #if the distance from the root is a minimum value , that element is the goal
                goal = elem
        heuristic_value = heuristic_value+minval*0.99
        currentbird =goal
        yellowbirds.remove(currentbird)
    return heuristic_value
